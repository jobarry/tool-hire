 <?php
include("header.php"); 
include("navBar.php")
?>

<div class="col-md-6 col-md-offset-3">
	<h1>Search Results:</h1>
	<div class="list-group">
        <a href="#" class="list-group-item">
        	<div class="row">
        	<div class="col-md-5">
        	<h4 class="list-group-item-heading">High Quality Hammer</h4>
        	<p class="list-group-item-text">This is a high quality hammer, only a year old. Excellent for...</p>
          	<p class="list-group-item-text">Distance: <strong>10 miles</strong></p>
     	    <p class="list-group-item-text">Price: <strong>£5/day</strong></p>
        	</div>
        	<div class="col-md-2">
        	<img style="width=10px" src="img/Hammer1.jpg">
        	</div>
        	</div>
        	<br>
			<h4><button type="button" class="btn btn-lg btn-primary">View</button></h4>
        </a>
        <a href="#" class="list-group-item">
            <div class="row">
        	<div class="col-md-5">
          	<h4 class="list-group-item-heading">Big Hammer</h4>
          	<p class="list-group-item-text">This a large hammer. Useful for hammering large objects...</p>
          	<p class="list-group-item-text">Distance: <strong>3 miles</strong></p>
     	    <p class="list-group-item-text">Price: <strong>£4/day</strong></p>
          	</div>
        	<div class="col-md-2">
        	<img src="img/Hammer2.jpg">
        	</div>
        	</div>
        	<br>
			<h4><button type="button" class="btn btn-lg btn-primary">View</button></h4>
        </a>
        <a href="#" class="list-group-item">
        	<div class="row">
        	<div class="col-md-5">
          	<h4 class="list-group-item-heading">Classic Hammer</h4>
          	<p class="list-group-item-text">The only hammer I've ever owned. It has never let me down...</p>
          	<p class="list-group-item-text">Distance: <strong>5 miles</strong></p>
     	    <p class="list-group-item-text">Price: <strong>£3/day</strong></p>
          	</div>
        	<div class="col-md-2">
        	<img src="img/Hammer3.jpg">
        	</div>
        	</div>
        	<br>
			<h4><button type="button" class="btn btn-lg btn-primary">View</button></h4>
        </a>
        <a href="#" class="list-group-item">
        	<div class="row">
        	<div class="col-md-5">
          	<h4 class="list-group-item-heading">Nice Hammer</h4>
          	<p class="list-group-item-text">Does the job. It hammers stuff.</p>
          	<p class="list-group-item-text">Distance: <strong>20 miles</strong></p>
     	    <p class="list-group-item-text">Price: <strong>£1/day</strong></p>
          	</div>
        	<div class="col-md-2">
        	<img src="img/Hammer4.jpg">
        	</div>
        	</div>
        	<br>
			<h4><button type="button" class="btn btn-lg btn-primary">View</button></h4>
        </a>
    </div>
</div>


 <?php
 include("footer.php");
 ?>